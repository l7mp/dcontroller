...
var _ = Describe("Controller stress tests", Ordered, func() {
	const (
		chainLength = 25
	...
	It("should setup a controller chain", func() {
...
target:
  ...
  kind: view`
	...
	It("should process an added object through controller chain", func() {
	...
	It("should process a deleted object through controller chain", func() {
		// Create the deleted view object with label "state: ctrl-1"
		view := object.NewViewObject("test", "view")
		object.SetName(view, "default", viewName)
		view.SetLabels(map[string]string{
			"state": "ctrl-1",
		...
		// Delete the view to the view cache
		err := vcache.Delete(view)
		Expect(err).NotTo(HaveOccurred())

		log.Info("Deleted initial object", "labels", view.GetLabels())
